<div class="container">
    <div class="userEdit">
      <ul class="nav nav-tabs">
        <li class="nav-item"><a class="nav-link active" href="#frmEditA" data-toggle="tab">INFORMATION</a></li>
        <li class="nav-item"><a class="nav-link" href="#frmEditB" data-toggle="tab">PASSWORD</a></li>
      </ul>
      <form class="frmEdit" action="{{route('users.update-user', Auth::user()->id)}}" method="POST" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="tab-content">
          <div class="tab-pane active" id="frmEditA">
            <div class="form-group">
              <label for="">User Name</label>
            <input class="txtUserName form-control" type="text" id="" name="name" value="{{Auth::user()->name}}" minlength="5"/>
              @if($errors->has('name'))
              <span style="margin-left: 2rem" class="text text-danger">{{$errors->first('name')}}</span>
              @endif
              <label for="">Email</label>
              <input class="txtEmail form-control" type="email" id="" name="email" value="{{Auth::user()->email}}" />
              @if($errors->has('email'))
              <span style="margin-left: 2rem" class="text text-danger">{{$errors->first('email')}}</span>
              @endif
              <label for="">Tel</label>
              <input class="txtTel form-control" type="text" id="" name="tel" />
            </div>
          </div>
          <div class="tab-pane" id="frmEditB">
            <div class="form-group">
              <label for="">Old Password</label>
              <input class="txtOldPassword form-control" type="password" id="" name="oldPassword" maxlength="10" value="" />
              <label for="">New Password</label>
              <input class="txtNewPassword form-control" type="password" id="" name="newPassword" />
              @if($errors->has('newPassword'))
              <span style="margin-left: 2rem" class="text text-danger">{{$errors->first('newPassword')}}</span>
              @endif
              <label for="">Confirm Password</label>
              <input class="txtConfirmPassword form-control" type="password" id="" name="newPassword_confirmation"/>
              @if($errors->has('newPassword_confirmation'))
              <span style="margin-left: 2rem" class="text text-danger">{{$errors->first('newPassword_confirmation')}}</span>
              @endif
            </div>
          </div>
          <div class="btnEdit">
            <input class="btn btn-danger" type="submit" id="" name="" value="Edit"/>
          </div>
        </div>
      </form>
    </div>
  </div>